...
router = APIRouter(prefix="/v1/api/produtos", tags=["Produtos"] ,dependencies=[Depends(auth)])


@router.post("/create", response_model=ProdutoResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/list_all", response_model=list[ProdutoResponse])
...
@router.get("/search_for_id/{id}", response_model=ProdutoResponse)
