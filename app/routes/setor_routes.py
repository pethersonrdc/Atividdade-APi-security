...
router = APIRouter(prefix="/v1/api/setores", tags=["Setores"] ,dependencies=[Depends(auth)])



@router.post("/create", response_model=SetorResponse, status_code=status.HTTP_201_CREATED)
...
@router.get("/list_all", response_model=list[SetorResponse])
...
@router.get("/search_for_id/{id}", response_model=SetorResponse)
